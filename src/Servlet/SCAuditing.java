...
/**
 * Servlet implementation class SCAuditing
 */
@WebServlet("/SCAuditing")
public class SCAuditing extends HttpServlet {
		...
		try {
			...
			while(objRS.next())
			{
				...
				for(int i = 1; i<=objMeta.getColumnCount(); i++ )
				{
					...
					if(objMeta.getColumnName(i).equals("Auditing")  && objRS.getInt(i)== 0){ 
						
						objJsonObject.put(objMeta.getColumnName(i), "已审核");
					}else if(objMeta.getColumnName(i).equals("Auditing") && objRS.getInt(i)== 1){
						
						objJsonObject.put(objMeta.getColumnName(i), "未审核");
