...
public class Uploadimgs extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		private String makePath(String filename, String savePath) {
			...
			SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM");//格式化当前日
			...
			try {
				...
				for (FileItem item : list) {
					...
					if (item.isFormField()) {
						...
						// 删除处理文件上传时生成的临时文件
						// item.delete();
						if(fileExtName.equals("jpeg") || fileExtName.equals("jpg") || fileExtName.equals("png") || fileExtName.equals("gif")  ){
							UploadBean objUploadBean =new UploadBean();
							uploadimg objUploadimgs = new uploadimg();
							Date now = new Date(); //获取当前时间
							SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM");//格式化当前日
							
							String dates  = dateFormat.format(now);
							objUploadBean.setImgurl("NewsOS/upload/img/"+dates+"/"+saveFilename);
							System.out.println("NewsOS/upload/img/"+dates+"/"+saveFilename);
							objUploadBean.setUpimgdepict(request.getParameter("depict"));
							nowtime objnowtime =new nowtime();
							objUploadBean.setImgtime(objnowtime.nowtime());
							objUploadimgs.Addimg(objUploadBean);
							objJsonObject.put("message", 1);
						}else{
							objJsonObject.put("message", 5);
						}
